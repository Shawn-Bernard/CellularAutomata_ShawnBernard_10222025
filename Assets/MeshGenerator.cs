...
public class MeshGenerator : MonoBehaviour
{
    ...
    /// <summary>
    /// Checks if the triangle is already in the Dictionary if not then add triangle  
    /// </summary>
    /// <param name="vertexIndedx"></param>
    /// <param name="triangle"></param>
    void AddTriangleToDictionary(int vertexIndedx, Triangle triangle)
    {
        if (triangleDictionary.ContainsKey(vertexIndedx))
        {
            triangleDictionary[vertexIndedx].Add(triangle);
        }
        else
        {
            List<Triangle> triangleList = new List<Triangle>();
            triangleList.Add(triangle);
            triangleDictionary.Add(vertexIndedx, triangleList);
        }
    }
    ...
    {
        List<Triangle> trianglesContainVertex = triangleDictionary[vertexIndex];

        for (int i = 0; i < trianglesContainVertex.Count; i++)
        {
            Triangle triangle = trianglesContainVertex[i];
            for (int j = 0; j < 3; j++)
            {
    ...
    {
        List<Triangle> trianglesContainVertexA = triangleDictionary[vertexA];
        int sharedTriangleCount = 0;

        for (int i = 0; i < trianglesContainVertexA.Count; i++)
        {
            if (trianglesContainVertexA[i].Contains(vertexB))
            {
                sharedTriangleCount++;
    ...
    public class SquareGrid
    {
        ...
        public SquareGrid(int[,] map, float squareSize)
        {
            ...
            for (int x = 0; x < nodeCountX - 1; x++)
            {
                ...
                {
                    squares[x,y] = new Square(controlNodes[x, y + 1], //Top left control node
                        controlNodes[x + 1, y + 1], //Top right control node
                        controlNodes[x + 1, y], //bottom right control node
                        controlNodes[x,y]);// bottom left control node
                }
            }
        }
    }
    ...
    public class Square
    {
        ...
        // !Fixed swaped _bottomRight <-> _bottomLeft
